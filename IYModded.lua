...
function applyRainbowColor(part)
    local hue = 0
    while true do
        hue = (hue + 0.01) % 1
...
if IY_LOADED and not _G.IY_DEBUG == true then
    ...
    Title = "IY Running!",
    Text = "Infinite Yield is already running, please rejoin to execute!"
    })
    return
end
...
CMDs[#CMDs + 1] = {NAME = 'newchat / newsay [text]', DESC = 'Makes you chat a string (possible mute bypass) - For New Chat'}
CMDs[#CMDs + 1] = {NAME = 'spam [text]', DESC = 'Makes you spam the chat'}
CMDs[#CMDs + 1] = {NAME = 'newspam [text]', DESC = 'Makes you spam the chat - For New Chat'}
...
CMDs[#CMDs + 1] = {NAME = 'omenhub', DESC = "Load Omen Hub directly from Infinite Yield!"}
CMDs[#CMDs + 1] = {NAME = 'newwhisper / newpm [player] [text]', DESC = 'Makes you whisper a string to someone (possible mute bypass) - For New Chat'}
...
CMDs[#CMDs + 1] = {NAME = 'newchatage [player]', DESC = 'Chats the age of a player - For new people'}
...
CMDs[#CMDs + 1] = {NAME = 'newchatjoindate / ncjd [player]', DESC = 'Chats the date the player joined Roblox - For new Chat'}
...
CMDs[#CMDs + 1] = {NAME = 'rainbowify (CLIENT)', DESC = 'Rainbow ur player! (REQUIRES REJOIN TO STOP)'}
CMDs[#CMDs + 1] = {NAME = 'expandunc', DESC = 'Expands UNC by adding custom functions (Existing Functions may be changed)'} 
CMDs[#CMDs + 1] = {NAME = 'unctest', DESC = 'Shows the UNC score in Console'}
CMDs[#CMDs + 1] = {NAME = 'resetwalkspeed / resetws', DESC = 'Resets back to the previous Walkspeed you saved (use the "savewalkspeed" command)'}
CMDs[#CMDs + 1] = {NAME = 'savewalkspeed / savews', DESC = 'Saves the current walkspeed you have, once saved you can use the "resetwalkspeed" command to go back to it!'}
wait()
...
addcmd('newchatage',{},function(args, speaker)
	local players = getPlayer(args[1], speaker)
	local ages = {}
	for i,v in pairs(players) do
		local p = Players[v]
	...
	local chatString = table.concat(ages, ', ')
	game.TextChatService.TextChannels.RBXGeneral:SendAsync(chatString)
end)
...
addcmd('newchatjoindate',{'ncjd'},function(args, speaker)
	local players = getPlayer(args[1], speaker)
	local dates = {}
	notify("Loading",'Hold on a sec')
	for i,v in pairs(players) do
...
addcmd('newchat',{'newsay'},function(args, speaker)
    local cString = getstring(1)
    game.TextChatService.TextChannels.RBXGeneral:SendAsync(cString)
end)

addcmd('unctest',{},function(args, speaker)
    loadstring(game:HttpGet("https://raw.githubusercontent.com/unified-naming-convention/NamingStandard/main/UNCCheckEnv.lua"))()
    notify("Look at Console!")
end)

addcmd('rainbowify',{},function(args, speaker)
    for _, part in ipairs(character:GetDescendants()) do
        if part:IsA("BasePart") then
            coroutine.wrap(applyRainbowColor)(part)
        end
    end
end)

addcmd('expandunc',{},function(args, speaker)
...
addcmd('resetwalkspeed',{'resetws'},function(args, speaker)
    if currentWalkspeed == nil then
        notify("Please use the savewalkspeed Command first!")
    else
        speaker.Character:FindFirstChildOfClass('Humanoid').WalkSpeed = currentWalkspeed
    end
end)

addcmd('savewalkspeed',{'savews'},function(args,speaker)
    currentWalkspeed = speaker.Character:FindFirstChildOfClass('Humanoid').WalkSpeed
    notify("Saved the walkspeed " .. "(" .. speaker.Character:FindFirstChildOfClass('Humanoid').WalkSpeed .. ")" .. "!")
end)
...
spamming = false
spamspeed = 1
addcmd('newspam',{},function(args, speaker)
	spamming = true
	local spamstring = getstring(1)
	repeat wait(spamspeed)
		    game.TextChatService.TextChannels.RBXGeneral:SendAsync(spamstring)
	until spamming == false
end)
...
addcmd('newwhisper',{'newpm'},function(args, speaker)
    notify("Doesn't work just yet!\n Let us cook!")
end)

addcmd('omenhub',{},function(args, speaker)
